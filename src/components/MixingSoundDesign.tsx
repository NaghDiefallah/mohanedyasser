        ...
        <ViewportReveal className="text-center mb-6 md:mb-8 space-y-3 md:space-y-4">
          <div className="flex items-center justify-center gap-4">
            ...
            <span className="text-primary uppercase tracking-[0.2em] md:tracking-[0.3em] text-[10px] md:text-xs font-bold">
              {t.mixingSoundDesign?.label || "Mixing and Sound Design"}
            </span>
            <div className="h-px w-8 md:w-12 bg-primary" />
          </div>
          
          <h2 className="text-2xl sm:text-4xl md:text-6xl font-black text-foreground uppercase tracking-tight">
            <span className="whitespace-nowrap">{t.mixingSoundDesign?.title || "MIXING"}</span>{" & "}
            <span className="text-primary whitespace-nowrap">
              {t.mixingSoundDesign?.titleHighlight || "SOUND DESIGN"}
            </span>
          </h2>
          
          <p className="text-muted-foreground max-w-md mx-auto text-sm md:text-base">
            {t.mixingSoundDesign?.description || "Drag the slider to see the before and after transformation."}
          </p>
        </ViewportReveal>
        ...
        <ViewportReveal>
          ...
          >
            ...
            >
              ...
              >
                {sliderPosition > 50
                  ? (t.mixingSoundDesign?.after || "AFTER")
                  : (t.mixingSoundDesign?.before || "BEFORE")
                }
              </span>
            </div>
          </div>
...
export default MixingSoundDesign;